...
let lift_binary_operation = function<a,b> (r:SourceRange,a: ExprRt<Sum<Val,Val>>, b:ExprRt<Sum<Val,Val>>, check_types:(ab:Prod<Val,Val>) => Sum<Prod<a,b>, Unit>, actual_operation:(_:Prod<a,b>)=>Val, operator_name?:string): ExprRt<Sum<Val,Val>> {
  return a.then(a_val => b.then(b_val =>
    apply(fun(check_types).then((fun(actual_operation).then(inl<Val,Val>()).then(fun<Sum<Val,Val>, ExprRt<Sum<Val,Val>>>(co_unit))).plus(
      constant<Unit,ExprRt<Sum<Val,Val>>>(runtime_error(r, `Type error: cannot perform ${operator_name} on ${a_val.value.v} and ${b_val.value.v}.`)))), { fst:a_val.value, snd:b_val.value })))
}
...
export let tuple_get_rt = (r:SourceRange, t:ExprRt<Sum<Val,Val>>, item_index:number) : ExprRt<Sum<Val,Val>> => {
  return t.then(t_val =>
        ...
        : runtime_error(r, `Type error (tuple): cannot lookup item ${item_index} on non-tuple value ${t_val.value}.`))
}
...
export let record_get_rt = (r:SourceRange, t:ExprRt<Sum<Val,Val>>, F_name:string) : ExprRt<Sum<Val,Val>> => {
  return t.then(t_val =>
        ...
        : runtime_error(r, `Type error (record): cannot lookup item ${F_name} on ${JSON.stringify( t_val.value)}.`))
}

export let render_surface_plus_rt = function (sr:SourceRange, r: ExprRt<Sum<Val, Val>>, p:ExprRt<Sum<Val, Val>>): ExprRt<Sum<Val, Val>> {
  return lift_binary_operation<RenderSurface,RenderSurfaceOperation>(sr, r, p,
...
export let mk_empty_render_surface_rt = function (r:SourceRange,width: ExprRt<Sum<Val,Val>>, height:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return width.then(w => height.then(h => color.then(col =>
    ...
    : runtime_error(r, `Type error: cannot create empty render surface with ${w.value.v}, ${h.value.v}, and ${col.value.v}.`)
  )))
}

export let mk_circle_rt = function (sr:SourceRange, x: ExprRt<Sum<Val,Val>>, y:ExprRt<Sum<Val,Val>>, r:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return x.then(x_v => y.then(y_v => r.then(r_v => color.then(col =>
    ...
    : runtime_error(sr, `Type error: cannot create circle with ${x_v.value.v}, ${y_v.value.v}, ${r_v.value.v} and ${col.value.v}.`)
  ))))
}

export let mk_square_rt = function (r:SourceRange, x: ExprRt<Sum<Val,Val>>, y:ExprRt<Sum<Val,Val>>, s:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return x.then(x_v => y.then(y_v => s.then(s_v => color.then(col => rot.then(rot_v =>
    ...
    : runtime_error(r, `Type error: cannot create square with ${x_v.value.v}, ${y_v.value.v}, ${s_v.value.v} and ${col.value.v}.`)
  )))))
}

export let mk_rectangle_rt = function (r:SourceRange, x: ExprRt<Sum<Val,Val>>, y:ExprRt<Sum<Val,Val>>, w:ExprRt<Sum<Val,Val>>, h:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return x.then(x_v => y.then(y_v => w.then(w_v => h.then(h_v => color.then(col => rot.then(rot_v =>
    ...
    : runtime_error(r, `Type error: cannot create rectangle with ${x_v.value.v}, ${y_v.value.v}, ${w_v.value.v}, ${h_v.value.v} and ${col.value.v}.`)
  ))))))
}

export let mk_line_rt = function (r:SourceRange, x1: ExprRt<Sum<Val,Val>>, y1:ExprRt<Sum<Val,Val>>, x2: ExprRt<Sum<Val,Val>>, y2:ExprRt<Sum<Val,Val>>, w:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return x1.then(x1_v => y1.then(y1_v => x2.then(x2_v => y2.then(y2_v => w.then(w_v => color.then(col => rot.then(rot_v =>
    ...
    : runtime_error(r, `Type error: cannot create line with ${x1_v.value.v}, ${y1_v.value.v}, ${x2_v.value.v}, ${y2_v.value.v}, ${w_v.value.v}, and ${col.value.v}.`)
  )))))))
}

export let mk_polygon_rt = function(r:SourceRange, points:ExprRt<Sum<Val,Val>>, col:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>) : ExprRt<Sum<Val,Val>> {
  return points.then(points_v =>
          col.then(col_v =>
          rot.then(rot_v =>
          points_v.value.k != "ref" ? runtime_error(r, `Type error: cannot create polygon with ${points_v.value.v}, ${col_v.value.v}, and ${rot_v.value.v}.`)
          :
          get_heap_v_rt(r, points_v.value.v).then(points_arr_v =>
          col_v.value.k == "s" && rot_v.value.k == "f" && points_arr_v.value.k == "arr" ?
            ...
            : runtime_error(r, `Type error: cannot create polygon with ${points_v.value.v}, ${col_v.value.v}, and ${rot_v.value.v}.`)
            ))))
}

export let mk_text_rt = function (r:SourceRange, text:ExprRt<Sum<Val,Val>>, x: ExprRt<Sum<Val,Val>>, y:ExprRt<Sum<Val,Val>>, s:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>, rotation:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return text.then(t_v => x.then(x_v => y.then(y_v => s.then(s_v => color.then(col => rotation.then(rot =>
    ...
    : runtime_error(r, `Type error: cannot create text with ${t_v.value.v}, ${x_v.value.v}, ${y_v.value.v}, ${s_v.value.v}, ${col.value.v} and ${rot.value.v}.`)
  ))))))
}


export let mk_sprite_rt = function (r:SourceRange, sprite: ExprRt<Sum<Val,Val>>, x: ExprRt<Sum<Val,Val>>, y:ExprRt<Sum<Val,Val>>, w:ExprRt<Sum<Val,Val>>, h:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return sprite.then(sprite_v => x.then(x_v => y.then(y_v => w.then(w_v => h.then(h_v => rot.then(rot_v =>
    ...
    : runtime_error(r, `Type error: cannot create sprite with ${x_v.value.v}, ${y_v.value.v}, ${w_v.value.v}, ${h_v.value.v} and ${rot_v.value.v}.`)
  ))))))
}

export let mk_ellipse_rt = function (r:SourceRange, x: ExprRt<Sum<Val,Val>>, y:ExprRt<Sum<Val,Val>>, w:ExprRt<Sum<Val,Val>>, h:ExprRt<Sum<Val,Val>>, color:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return x.then(x_v => y.then(y_v => w.then(w_v => h.then(h_v => color.then(col => rot.then(rot_v =>
    x_v.value.k == "f" && y_v.value.k == "f" && w_v.value.k == "f" && h_v.value.k == "f" && col.value.k == "s" && rot_v.value.k == "f" ?
      render_surface_operation_expr(mk_ellipse_op(x_v.value.v, y_v.value.v, w_v.value.v, h_v.value.v, col.value.v, rot_v.value.v))
    : runtime_error(r, `Type error: cannot create ellipse with ${x_v.value.v}, ${y_v.value.v}, ${w_v.value.v}, ${h_v.value.v} and ${col.value.v}.`)
  ))))))
}

export let mk_other_surface_rt = function (r:SourceRange, s:ExprRt<Sum<Val,Val>>, dx: ExprRt<Sum<Val,Val>>, dy:ExprRt<Sum<Val,Val>>, sx:ExprRt<Sum<Val,Val>>, sy:ExprRt<Sum<Val,Val>>, rot:ExprRt<Sum<Val,Val>>): ExprRt<Sum<Val, Val>> {
  return dx.then(dx_v => dy.then(dy_v => sx.then(sx_v => sy.then(sy_v => s.then(s_v => rot.then(rot_v =>
    ...
    : runtime_error(r, `Type error: cannot create other surface with ${dx_v.value.v}, ${dy_v.value.v}, ${sx_v.value.v}, ${sy_v.value.v}, ${s_v.value.v}, and ${rot_v.value.v}.`)
  ))))))
}

let lift_unary_operation = function<a> (r:SourceRange, a: ExprRt<Sum<Val, Val>>, check_type:(a:Val) => Sum<a, Unit>, actual_operation:(_:a)=>Val, operator_name?:string): ExprRt<Sum<Val, Val>> {
  return a.then(a_val =>
    apply(fun(check_type).then((fun(actual_operation).then(inl<Val,Val>()).then(fun<Sum<Val,Val>, ExprRt<Sum<Val, Val>>>(co_unit))).plus(constant<Unit,ExprRt<Sum<Val, Val>>>(
      runtime_error(r, `Type error: cannot perform ${operator_name} on value ${a_val.value.v}.`)))), a_val.value))
}
export let int_minus_unary_rt = function (r:SourceRange, a: ExprRt<Sum<Val, Val>>): ExprRt<Sum<Val, Val>> {
  return lift_unary_operation<number>(r, a,
          ab => ab.k != "i" ? inr<number, Unit>().f({}) : inl<number, Unit>().f(ab.v),
          ab_val => mk_int_val(-ab_val), "(-)")
}
export let float_minus_unary_rt = function (r:SourceRange, a: ExprRt<Sum<Val, Val>>): ExprRt<Sum<Val, Val>> {
  return lift_unary_operation<number>(r, a,
          ab => ab.k != "f" ? inr<number, Unit>().f({}) : inl<number, Unit>().f(ab.v),
          ab_val => mk_float_val(-ab_val), "(-)")
}
export let string_length_rt = function (r:SourceRange, a: ExprRt<Sum<Val, Val>>): ExprRt<Sum<Val, Val>> {
  return lift_unary_operation<string>(r, a,
