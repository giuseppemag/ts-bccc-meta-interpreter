...
import { Stmt, Mem, Err, Expr, set_highlighting, Bool, Val, runtime_error } from "./memory"
...
export let if_then_else = function(c:Expr<Val>, f:Stmt, g:Stmt) : Stmt {
  return c.then(c_val => c_val.k != "b" ? runtime_error<Unit>(`Error: conditional expression ${c_val} is not a boolean.`)
                         : c_val.v ? f : g)
}
