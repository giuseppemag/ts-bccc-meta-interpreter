...
export interface Lambda { body:ExprRt<Val>, parameters:Array<Name>, closure: Scope }
...
export interface Interface { base:Sum<Interface, Unit>, methods:Immutable.Map<Name, StmtRt> }
export let empty_scope_val = Immutable.Map<Name, Val>()
...
export let mk_ref_val : (_:Name) => Val = r => ({ v:r, k:"ref" })
...
export interface MemRt { highlighting:SourceRange, globals:Scope, heap:Scope, functions:Immutable.Map<Name,Lambda>, classes:Immutable.Map<Name, Interface>, stack:Immutable.Map<number, Scope> }
let highlight : Fun<Prod<SourceRange, MemRt>, MemRt> = fun(x => ({...x.snd, highlighting:x.fst }))
export let load: Fun<Prod<string, MemRt>, Sum<Unit,Val>> = fun(x =>
  ...
  : apply(inl<Unit,Val>(), {}))
export let store: Fun<Prod<Prod<string, Val>, MemRt>, MemRt> = fun(x =>
  !x.snd.stack.isEmpty() ?
...
export let load_class_def: Fun<Prod<Name, MemRt>, Sum<Unit,Interface>> = fun(x =>
  x.snd.classes.has(x.fst) ? apply(inr(), x.snd.classes.get(x.fst)) : apply(inl<Unit,Interface>(), {}))
export let store_class_def: Fun<Prod<Prod<Name, Interface>, MemRt>, MemRt> = fun(x => ({...x.snd, classes:x.snd.classes.set(x.fst.fst, x.fst.snd) }))
export let load_fun_def: Fun<Prod<Name, MemRt>, Sum<Unit,Lambda>> = fun(x =>
  x.snd.functions.has(x.fst) ?
    apply(inr(), x.snd.functions.get(x.fst))
  : apply(inl(), {}))
export let store_fun_def: Fun<Prod<Prod<Name, Lambda>, MemRt>, MemRt> = fun(x => ({...x.snd, functions:x.snd.functions.set(x.fst.fst, x.fst.snd) }))
export let load_heap: Fun<Prod<Name, MemRt>, Sum<Unit,Val>> = fun(x =>
  x.snd.heap.has(x.fst) ?
    apply(inr(), x.snd.heap.get(x.fst))
  : apply(inl(), {}))
export let store_heap: Fun<Prod<Prod<Name, Val>, MemRt>, MemRt> = fun(x => ({...x.snd, heap:x.snd.heap.set(x.fst.fst, x.fst.snd) }))
export let heap_alloc: Fun<Prod<Val,MemRt>, Prod<Val, MemRt>> = fun(x => {
  let new_ref = `ref_${x.snd.heap.count()}`
  return ({ fst:mk_ref_val(new_ref), snd:{...x.snd, heap:x.snd.heap.set(new_ref, x.fst) }})
})
export let push_scope = curry(fun2<Scope,MemRt,MemRt>((s,m) => ({...m, stack:m.stack.set(m.stack.count(), s)})))
export let pop_scope: Fun<MemRt, Sum<Unit,MemRt>> = fun(x =>
  !x.stack.isEmpty() ?
...
export let empty_memory:MemRt = { highlighting:mk_range(0,0,0,0), globals:empty_scope_val, heap:empty_scope_val, functions:Immutable.Map<Name,Lambda>(), classes:Immutable.Map<Name, Interface>(), stack:Immutable.Map<number, Scope>() }
...
export let set_v_expr_rt = function (v: Name, e: ExprRt<Val>): StmtRt {
  return e.then(e_val =>
    // console.log(`Setting ${v} to ${JSON.stringify(e_val)}`) ||
    set_v_rt(v, e_val))
}
export let set_v_rt = function (v: Name, val: Val): StmtRt {
  let store_co = store.then(constant<MemRt,Val>(mk_unit_val).times(id<MemRt>()).then(Co.value<MemRt, ErrVal, Val>().then(Co.result<MemRt, ErrVal, Val>().then(Co.no_error<MemRt, ErrVal, Val>()))))
  let f = ((constant<MemRt, string>(v).times(constant<MemRt, Val>(val))).times(id<MemRt>())).then(store_co)
  return mk_coroutine(f)
}
export let get_v_rt = function (v: Name): ExprRt<Val> {
  let f = constant<MemRt, string>(v).times(id<MemRt>()).then(load).times(id<MemRt>()).then(CCC.swap_prod()).then(CCC.distribute_sum_prod()).then(snd<MemRt,Unit>().map_plus(id()))
...
export let new_obj_rt = function (): ExprRt<Val> {
  let heap_alloc_co:Coroutine<MemRt,ErrVal,Val> = mk_coroutine(constant<MemRt,Val>(mk_obj_val(empty_scope_val)).times(id<MemRt>()).then(heap_alloc).then(Co.value<MemRt, ErrVal, Val>().then(Co.result<MemRt, ErrVal, Val>().then(Co.no_error<MemRt, ErrVal, Val>()))))
  return (heap_alloc_co)
}
export let new_arr_rt = function (len:number): ExprRt<Val> {
  let heap_alloc_co:Coroutine<MemRt,ErrVal,Val> = mk_coroutine(constant<MemRt,Val>(mk_arr_val(init_array_val(len))).times(id<MemRt>()).then(heap_alloc).then(Co.value<MemRt, ErrVal, Val>().then(Co.result<MemRt, ErrVal, Val>().then(Co.no_error<MemRt, ErrVal, Val>()))))
  return (heap_alloc_co)
}
...
export let set_heap_v_rt = function (v: Name, val: Val): StmtRt {
  let store_co = store_heap.then(constant<MemRt,Val>(mk_unit_val).times(id<MemRt>()).then(Co.value<MemRt, ErrVal, Val>().then(Co.result<MemRt, ErrVal, Val>().then(Co.no_error<MemRt, ErrVal, Val>()))))
  let f = ((constant<MemRt, string>(v).times(constant<MemRt, Val>(val))).times(id<MemRt>())).then(store_co)
  return mk_coroutine(f)
}
export let get_heap_v_rt = function (v: Name): ExprRt<Val> {
  let f = (constant<MemRt, string>(v).times(id<MemRt>()).then(load_heap)).times(id<MemRt>()).then(swap_prod()).then(CCC.distribute_sum_prod()).then(snd<MemRt,Unit>().map_plus(swap_prod()))
...
export let set_class_def_rt = function (v: Name, int: Interface): StmtRt {
  let store_co = store_class_def.then(constant<MemRt,Val>(mk_unit_val).times(id<MemRt>()).then(Co.value<MemRt, ErrVal, Val>().then(Co.result<MemRt, ErrVal, Val>().then(Co.no_error<MemRt, ErrVal, Val>()))))
  let f = ((constant<MemRt, string>(v).times(constant<MemRt, Interface>(int))).times(id<MemRt>())).then(store_co)
  return mk_coroutine(f)
}
export let get_class_def_rt = function (v: Name): ExprRt<Interface> {
  let f = (constant<MemRt, string>(v).times(id<MemRt>()).then(load_class_def)).times(id<MemRt>()).then(
...
export let set_fun_def_rt = function (v: Name, l: Lambda): StmtRt {
  let store_co = store_fun_def.then(constant<MemRt,Val>(mk_unit_val).times(id<MemRt>()).then(Co.value<MemRt, ErrVal, Val>().then(Co.result<MemRt, ErrVal, Val>().then(Co.no_error<MemRt, ErrVal, Val>()))))
  let f = ((constant<MemRt, string>(v).times(constant<MemRt, Lambda>(l))).times(id<MemRt>())).then(store_co)
  return mk_coroutine(f)
}
export let get_fun_def_rt = function (v: Name): ExprRt<Lambda> {
  let f = (constant<MemRt, string>(v).times(id<MemRt>()).then(load_fun_def)).times(id<MemRt>()).then(swap_prod()).then(CCC.distribute_sum_prod()).then(snd<MemRt,Unit>().map_plus(swap_prod()))
