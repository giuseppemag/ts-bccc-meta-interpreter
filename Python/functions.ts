...
let build_closure = (closure_parameters:Array<Name>) => function(i:number, closure:Scope) : ExprRt<Scope> {
...
export let mk_lambda_rt = function(body:ExprRt<Val>, parameters:Array<Name>, closure_parameters:Array<Name>) : ExprRt<Val> {
...
export let def_fun_rt = function(n:Name, body:ExprRt<Val>, parameters:Array<Name>, closure_parameters:Array<Name>) : StmtRt {
...
export let call_by_name_rt = function(f_n:Name, args:Array<ExprRt<Val>>) : ExprRt<Val> {
...
export let call_lambda_rt = function(lambda:Lambda, arg_expressions:Array<ExprRt<Val>>) : ExprRt<Val> {
  ...
  let set_args = (arg_values:Array<Val>) => lambda.parameters.map((n,i) => ({ fst:n, snd:arg_values[i] })).reduce<StmtRt>((sets, arg_value) =>
    ...
    done_rt)
  let init = mk_coroutine(apply(push_scope, lambda.closure).then(unit<MemRt>().times(id<MemRt>())).then(Co.value<MemRt, ErrVal, Unit>().then(Co.result<MemRt, ErrVal, Unit>().then(Co.no_error<MemRt, ErrVal, Unit>()))))
