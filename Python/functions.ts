...
export let call_lambda = function(lambda:Lambda, arg_expressions:Array<Expr<Val>>) : Expr<Val> {
  ...
  let eval_args:Coroutine<Mem, Err, Immutable.List<Val>> = comm_list_coroutine(Immutable.List<Expr<Val>>(arg_expressions))

  let set_args = (arg_values:Array<Val>) => lambda.parameters.map((n,i) => ({ fst:n, snd:arg_values[i] })).reduce<Stmt>((sets, arg_value) =>
    set_v(arg_value.fst, arg_value.snd).then(_ => sets),
    done)
