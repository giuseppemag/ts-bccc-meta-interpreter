...
interface DocumentBlockData<k> { kind:k, content:string, index:number }
...
let document_editor = (mode:Mode) : C<void> => {
  type BlockKind = "markdown" | "latex" | "image" | "code"
  ...
  let raw_blocks:Array<[BlockKind, ActualDocumentBlock]> = [
    ["code", { kind:"code", default_content:default_program, renderer:(block_data:ActualDocumentBlockData) => label<string,string>("Code", true, undefined, `block-label-${block_data.index}`)(
      content => custom<string>()(
        _ => k => <CodeComponent code={content} set_content={c => k(() => {})(c)} mode={mode} />)
    )(block_data.content) }],
    ["markdown", { kind:"markdown", default_content:"", renderer:(block_data:ActualDocumentBlockData) => label<string,string>("Markdown", true, undefined, `block-label-${block_data.index}`)(
      content => custom<string>()(
        _ => k => <MarkupComponent content={content} set_content={c => k(() => {})(c)} mode={mode} kind="Markdown" />)
    )(block_data.content) }],
    ["latex", { kind:"latex", default_content:"", renderer:(block_data:ActualDocumentBlockData) => label<string,string>("Latex", true, undefined, `block-label-${block_data.index}`)(
      content => custom<string>()(
        _ => k => <MarkupComponent content={content} set_content={c => k(() => {})(c)} mode={mode} kind="LaTeX" />))(block_data.content) }],
    ["image", { kind:"image", default_content:"", renderer:(block_data:ActualDocumentBlockData) => label<string,string>("Image", true, undefined, `block-label-${block_data.index}`)(
    ...
    any<EditorState, EditorState>("document-editor-main")([
      ...
      retract<EditorState,ActualDocument>("document-editor-document-retract")(e => e.document, e => d => ({...e, document:d}),
        d => any<ActualDocument, ActualDocument>("document-editor-blocks")(
          d.blocks.sortBy((v,k) => v && v.index).map((b,b_i) => {
            if (!b || !b_i) return _ => unit(null).never<ActualDocument>()
            return d => blocks.get(b.kind).renderer(b).then(`block-${b_i}`, new_content =>
                  unit<ActualDocument>({...d, blocks:d.blocks.set(b_i, {...d.blocks.get(b_i), content:new_content }) }) )
          }).toArray()
        )(d)),
      retract<EditorState,ActualDocument>("document-editor-new-block-retract")(e => e.document, e => d => ({...e, document:d}),
        ...
        raw_blocks.map(rb => d =>
          ...
          unit<ActualDocument>({...d, next_key:d.next_key+1, blocks:d.blocks.set(d.next_key, { kind:rb[1].kind, index:d.blocks.count(), content:rb[1].default_content })}))
        )
      ))
    ]
    )
  )({ document:{ blocks:Immutable.Map<number,ActualDocumentBlockData>(), next_key:1 }, current_path:none<string>() }).never()
}
