...
exports.def_fun = function (n, body, args) {
    return memory_1.set_fun_def(n, ts_bccc_1.apply(ts_bccc_1.constant(body).times(ts_bccc_1.constant(args)), {}));
};
...
exports.call_lambda = function (lambda, arg_values) {
    var body = lambda.fst;
    if (arg_values.length != lambda.snd.length)
        return memory_1.runtime_error("Error: wrong number of parameters in lambda invocation. Expected " + lambda.snd.length + ", received " + arg_values.length + ".");
    var set_args = lambda.snd.map(function (n, i) { return ({ fst: n, snd: arg_values[i] }); }).reduce(function (sets, arg_expr) {
        return memory_1.set_v_expr(arg_expr.fst, arg_expr.snd).then(function (_) { return sets; });
    }, basic_statements_1.done);
    var init = ts_bccc_2.mk_coroutine(memory_1.push_scope.then(ts_bccc_1.unit().times(ts_bccc_1.id())).then(Co.value().then(Co.result().then(Co.no_error()))));
