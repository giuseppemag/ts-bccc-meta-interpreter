...
exports.render_surface_plus_rt = function (sr, r, p) {
    return lift_binary_operation(sr, r, p, function (ab) { return ab.fst.k != "render surface" || ab.snd.k != "render surface operation" ? ts_bccc_1.inr().f({}) : ts_bccc_1.inl().f({ fst: ab.fst.v, snd: ab.snd.v }); }, function (ab_val) { return python_1.mk_render_surface_val(__assign({}, ab_val.fst, { operations: ab_val.fst.operations.push(ab_val.snd) })); }, "(+)");
};
exports.mk_empty_render_surface_rt = function (r, width, height, color) {
    return width.then(function (w) { return height.then(function (h) { return color.then(function (col) {
        return w.value.k == "f" && h.value.k == "f" && col.value.k == "s" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create empty render surface with " + w.value.v + ", " + h.value.v + ", and " + col.value.v + ".");
    }); }); });
};
exports.mk_circle_rt = function (sr, x, y, r, color) {
    return x.then(function (x_v) { return y.then(function (y_v) { return r.then(function (r_v) { return color.then(function (col) {
        return x_v.value.k == "f" && y_v.value.k == "f" && r_v.value.k == "f" && col.value.k == "s" ?
            ...
            : memory_1.runtime_error(sr, "Type error: cannot create circle with " + x_v.value.v + ", " + y_v.value.v + ", " + r_v.value.v + " and " + col.value.v + ".");
    }); }); }); });
};
exports.mk_square_rt = function (r, x, y, s, color, rot) {
    return x.then(function (x_v) { return y.then(function (y_v) { return s.then(function (s_v) { return color.then(function (col) { return rot.then(function (rot_v) {
        return x_v.value.k == "f" && y_v.value.k == "f" && s_v.value.k == "f" && col.value.k == "s" && rot_v.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create square with " + x_v.value.v + ", " + y_v.value.v + ", " + s_v.value.v + " and " + col.value.v + ".");
    }); }); }); }); });
};
exports.mk_rectangle_rt = function (r, x, y, w, h, color, rot) {
    return x.then(function (x_v) { return y.then(function (y_v) { return w.then(function (w_v) { return h.then(function (h_v) { return color.then(function (col) { return rot.then(function (rot_v) {
        return x_v.value.k == "f" && y_v.value.k == "f" && w_v.value.k == "f" && h_v.value.k == "f" && col.value.k == "s" && rot_v.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create rectangle with " + x_v.value.v + ", " + y_v.value.v + ", " + w_v.value.v + ", " + h_v.value.v + " and " + col.value.v + ".");
    }); }); }); }); }); });
};
exports.mk_line_rt = function (r, x1, y1, x2, y2, w, color, rot) {
    return x1.then(function (x1_v) { return y1.then(function (y1_v) { return x2.then(function (x2_v) { return y2.then(function (y2_v) { return w.then(function (w_v) { return color.then(function (col) { return rot.then(function (rot_v) {
        return x1_v.value.k == "f" && y1_v.value.k == "f" && x2_v.value.k == "f" && y2_v.value.k == "f" && w_v.value.k == "f" && col.value.k == "s" && rot_v.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create line with " + x1_v.value.v + ", " + y1_v.value.v + ", " + x2_v.value.v + ", " + y2_v.value.v + ", " + w_v.value.v + ", and " + col.value.v + ".");
    }); }); }); }); }); }); });
};
exports.mk_polygon_rt = function (r, points, col, rot) {
    return points.then(function (points_v) {
        return col.then(function (col_v) {
            return rot.then(function (rot_v) {
                return points_v.value.k != "ref" ? memory_1.runtime_error(r, "Type error: cannot create polygon with " + points_v.value.v + ", " + col_v.value.v + ", and " + rot_v.value.v + ".")
                    :
                        memory_1.get_heap_v_rt(r, points_v.value.v).then(function (points_arr_v) {
                            return col_v.value.k == "s" && rot_v.value.k == "f" && points_arr_v.value.k == "arr" ?
                                ...
                                : memory_1.runtime_error(r, "Type error: cannot create polygon with " + points_v.value.v + ", " + col_v.value.v + ", and " + rot_v.value.v + ".");
                        });
            });
        });
    });
};
exports.mk_text_rt = function (r, text, x, y, s, color, rotation) {
    return text.then(function (t_v) { return x.then(function (x_v) { return y.then(function (y_v) { return s.then(function (s_v) { return color.then(function (col) { return rotation.then(function (rot) {
        return t_v.value.k == "s" && x_v.value.k == "f" && y_v.value.k == "f" && s_v.value.k == "f" && col.value.k == "s" && rot.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create text with " + t_v.value.v + ", " + x_v.value.v + ", " + y_v.value.v + ", " + s_v.value.v + ", " + col.value.v + " and " + rot.value.v + ".");
    }); }); }); }); }); });
};
exports.mk_sprite_rt = function (r, sprite, x, y, w, h, rot) {
    return sprite.then(function (sprite_v) { return x.then(function (x_v) { return y.then(function (y_v) { return w.then(function (w_v) { return h.then(function (h_v) { return rot.then(function (rot_v) {
        return sprite_v.value.k == "s" && x_v.value.k == "f" && y_v.value.k == "f" && w_v.value.k == "f" && h_v.value.k == "f" && rot_v.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create sprite with " + x_v.value.v + ", " + y_v.value.v + ", " + w_v.value.v + ", " + h_v.value.v + " and " + rot_v.value.v + ".");
    }); }); }); }); }); });
};
exports.mk_ellipse_rt = function (r, x, y, w, h, color, rot) {
    return x.then(function (x_v) { return y.then(function (y_v) { return w.then(function (w_v) { return h.then(function (h_v) { return color.then(function (col) { return rot.then(function (rot_v) {
        return x_v.value.k == "f" && y_v.value.k == "f" && w_v.value.k == "f" && h_v.value.k == "f" && col.value.k == "s" && rot_v.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create ellipse with " + x_v.value.v + ", " + y_v.value.v + ", " + w_v.value.v + ", " + h_v.value.v + " and " + col.value.v + ".");
    }); }); }); }); }); });
};
exports.mk_other_surface_rt = function (r, s, dx, dy, sx, sy, rot) {
    return dx.then(function (dx_v) { return dy.then(function (dy_v) { return sx.then(function (sx_v) { return sy.then(function (sy_v) { return s.then(function (s_v) { return rot.then(function (rot_v) {
        return dx_v.value.k == "f" && dy_v.value.k == "f" && sx_v.value.k == "f" && sy_v.value.k == "f" && s_v.value.k == "render surface" && rot_v.value.k == "f" ?
            ...
            : memory_1.runtime_error(r, "Type error: cannot create other surface with " + dx_v.value.v + ", " + dy_v.value.v + ", " + sx_v.value.v + ", " + sy_v.value.v + ", " + s_v.value.v + ", and " + rot_v.value.v + ".");
    }); }); }); }); }); });
};
...
exports.int_minus_unary_rt = function (r, a) {
    return lift_unary_operation(r, a, function (ab) { return ab.k != "i" ? ts_bccc_1.inr().f({}) : ts_bccc_1.inl().f(ab.v); }, function (ab_val) { return memory_1.mk_int_val(-ab_val); }, "(-)");
};
exports.float_minus_unary_rt = function (r, a) {
    return lift_unary_operation(r, a, function (ab) { return ab.k != "f" ? ts_bccc_1.inr().f({}) : ts_bccc_1.inl().f(ab.v); }, function (ab_val) { return memory_1.mk_float_val(-ab_val); }, "(-)");
};
exports.string_length_rt = function (r, a) {
    return lift_unary_operation(r, a, function (ab) { return ab.k != "s" ? ts_bccc_1.inr().f({}) : ts_bccc_1.inl().f(ab.v); }, function (ab_val) { return memory_1.mk_int_val(ab_val.length); }, "(length)");
};
