...
exports.get_fs = fast_coroutine_1.co_get_state().then(function (st) { return fast_coroutine_1.co_unit(st.fs); });
exports.set_fs = function (fs) {
    return fast_coroutine_1.co_get_state().then(function (st) {
        return fast_coroutine_1.co_set_state(__assign({}, st, { fs: fs }));
    });
};
exports.attr_map = function (attr0) {
    return fast_coroutine_1.comm_list_coroutine(attr0).then(function (attr1) {
        var attr2 = attr1.toArray().map(function (a) {
            var t = a.value.v;
            var kv = [t[0].v, t[1].v];
            return kv;
        });
        return fast_coroutine_1.co_unit(Immutable.Map(attr2));
    });
};
                ...
                return p_v.value.k !== "s"
                    ...
                    c_v.value.k !== "s"
                        ...
                        exports.set_fs(fs.set(p_v.value.v, { content: c_v.value.v })).then(function (_) {
                            return fast_coroutine_1.co_unit(ts_bccc_1.apply(ts_bccc_1.inl(), main_1.mk_unit_val));
                        });
            });
        });
    });
...
exports.set_file_from_block = function (r, path, attr) {
    return exports.attr_map(attr).then(function (attr_v) {
        return attr_v.has("content")
            ? exports.set_file(r, path, fast_coroutine_1.co_unit(ts_bccc_1.apply(ts_bccc_1.inl(), main_1.mk_string_val(attr_v.get("content")))))
            : fast_coroutine_1.co_error({ range: r, message: "Every file must specify a property 'content'" });
    });
};
    ...
    return exports.get_fs.then(function (fs) {
        return path.then(function (p_v) {
            return fs.has(p_v.value.v)
                ? fast_coroutine_1.co_unit(ts_bccc_1.apply(ts_bccc_1.inl(), main_1.mk_string_val(fs.get(p_v.value.v).content)))
...
exports.exists = function (path) {
    return path.then(function (p_v) {
        return exports.get_fs.then(function (fs) {
            return fast_coroutine_1.co_unit(ts_bccc_1.apply(ts_bccc_1.inl(), main_1.mk_bool_val(fs.has(p_v.value.v))));
        });
    });
};
exports.copy_file = function (r, path_from, path_to) {
    return path_from.then(function (pf_v) { return path_to.then(function (pt_v) {
        return exports.get_file(r, fast_coroutine_1.co_unit(pf_v)).then(function (f_v) {
            return exports.set_file(r, fast_coroutine_1.co_unit(pt_v), fast_coroutine_1.co_unit(f_v));
        });
    }); });
};
...
exports.init_fs = function (files) {
    return fast_coroutine_1.comm_list_coroutine(files).then(function (_) {
        return fast_coroutine_1.co_unit(ts_bccc_1.apply(ts_bccc_1.inl(), main_1.mk_unit_val));
    });
};
