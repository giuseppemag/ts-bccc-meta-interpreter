...
import { Stmt, State, Err, Typing, Type, TypeInformation, Parameter, LambdaDefinition, FunDefinition, MethodDefinition, CallingContext, FieldDefinition, Modifier, ObjType, GenericParameter, GenericMethodDefinition } from "./types";
...
export declare let def_class: (r: SourceRange, modifiers: Modifier[], C_kind: "interface" | "abstract" | "normal", C_name: string, extends_or_implements: {
    ...
    type: Type;
}[], methods_from_context: ((_: CallingContext) => MethodDefinition)[], methods_generic_from_context: ((_: CallingContext) => GenericMethodDefinition)[], fields_from_context: ((_: CallingContext) => FieldDefinition)[], is_internal?: boolean) => Stmt;
export declare let generic_instance_name: (C_name: string, generic_parameters: GenericParameter[], generic_arguments: Immutable.Map<string, Type>) => string;
export declare let def_generic_class: (r: SourceRange, C_name: string, generic_parameters: GenericParameter[], instantiate: (_: Immutable.Map<string, Type>, is_visible?: boolean | undefined) => Stmt) => Stmt;
export declare let field_get: (r: SourceRange, context: CallingContext, this_ref: Stmt, F_or_M_name: {
    name: string;
    params: Type[];
}, n?: number, called_by?: string) => Stmt;
export declare let field_set: (r: SourceRange, context: CallingContext, this_ref: Stmt, F_name: {
    ...
    kind: "att_arr";
    index: Stmt;
}, new_value: Stmt, called_by_constructor?: Sum<GenericMethodDefinition, Unit>) => Stmt;
