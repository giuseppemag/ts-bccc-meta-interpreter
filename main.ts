...
module ImpLanguageWithSuspend {
  ...
  let unit_expr = () => mk_expr(co_unit<Mem,Err,Val>(unt))
  let str_expr = (s:string) => mk_expr(co_unit<Mem,Err,Val>(str(s)))
  let int_expr = (n:number) => mk_expr(co_unit<Mem,Err,Val>(int(n)))
  let lambda_expr = (l:Prod<Expr<Val>, Array<Name>>) => mk_expr(co_unit<Mem,Err,Val>(lambda(l)))
  let obj_expr = (o:Scope) => mk_expr(co_unit<Mem,Err,Val>(obj(o)))
  let ref_expr = (r:Name) => mk_expr(co_unit<Mem,Err,Val>(ref(r)))
  let val_expr = (v:Val) => mk_expr(co_unit<Mem,Err,Val>(v))
  ...
  let done: Stmt = mk_stmt(apply(fun<Unit, Coroutine<Mem, Err, Unit>>(co_unit), {}))
  ...
  let call_by_name = function(f_n:Name, args:Array<Expr<Val>>) : Expr<Val> {
    return mk_expr(get_v(f_n).then(f => f.k == "lambda" ? call_lambda(f.v, args) : unit_expr()))
  }
  ...
  let call_cons = function(C_name:Name, args:Array<Expr<Val>>) : Expr<Val> {
    ...
    mk_expr(co_unit<Mem,Err,Val>(this_addr))
    )))))))
  }
...
export let test_imp = function () {
    let loop_test =
      ...
      while_do(mk_expr(get_v("n").then(n => co_unit(n.v > 0))),
    ...
    let class_test =
      mk_stmt(declare_class("Vector2", vector2).then(_ =>
      mk_expr(call_cons("Vector2", [int_expr(10), int_expr(20)]).then(v2 =>
      set_v("v2", v2).then(_ =>
      call_method("scale", v2, [int_expr(2)]).semicolon(
      done
      ))))))
